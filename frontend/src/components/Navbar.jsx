    ...
    <header className="navbar scrolled">
      <div className="container navbar-inner">
        ...
        <div className="navbar-actions">
          {user ? (
            <>
              <Link to={`/${user.role}/dashboard`} className="text-dim" style={{ fontSize: 14, fontWeight: 500 }}>
                Hi, {user.name}
              </Link>
              <Link to={`/${user.role}/dashboard`} className="btn btn-primary btn-sm">
                Dashboard
              </Link>
              <button onClick={onLogout} className="btn btn-secondary btn-sm" style={{ border: 'none', background: 'transparent' }}>
                Log out
              </button>
            </>
          ) : (
